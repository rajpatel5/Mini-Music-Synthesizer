...
#define FS 44100			// Defines the sampling frequency for note generation in Hz

typedef struct note_struct{	
	/*	This holds the data representing a single note */
	...
	double *waveform;		// A pointer to a dynamically generated array which will containthe note's waveform
	...
	int n_sampled;			// Counter indicating how many samples have been generated for this note
...
{
 // Creates and initializes a new note from the input values
 ...
 // Need to set up a dynamically allocated array to hold the note's waveform. 
...
note *playlist_insert(note *head, double freq, int bar, double index)
{
 ...
 // Adds a new note at the requested location. The node is
...
{
 /*
   Creates a unique int key for a note from bar and index. Note that
...
{
 /*
    Implements the Karplus-Strong plucked string
 ...
 */
 ...
 if (f)
 {
  ...
  // Keeps track of bar:index position and plays any notes
